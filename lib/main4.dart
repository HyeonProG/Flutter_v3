...
class _MyHomePageState extends State<MyHomePage> {
  ...
  // 통신을 담당하는 함수 --> MVC --> MVVM --> MVVM + 클린 아키텍쳐
  Future<void> _fetchTodos() async {
    // 통신 코드 작성시 반드시 예외 처리 코드를 작성해야한다.
    try {
      ...
      print('서버에서 전달된 데이터 확인: ${response.data}');
      print('dart언어에서 데이터 타입 확인 : ${response.data.runtimeType}');
      print('-------------------------------------------');
      print('넘겨받은 _JsonMap 구조에서 속성 값 확인1 : ${response.data['userId']}');
      print('넘겨받은 _JsonMap 구조에서 속성 값 확인2 : ${response.data['title']}');
      print('-------------------------------------------');
